...
Each unit gets 3 attack power and 200 hp

Reading order is top->down, left->right (English reading format).
Each turn:
	Map is scanned. All Elves and Goblins are identified in reading order.
	Each elf or goblin:
		Scans for all enemies
		Attacks if an an enemy is within range (1 square away N, S, E, or W )
			Turn ends
		Identifies the nearest (in the case of a tie the first in reading order wins)
		Identifies any open squares (Only N, S, E, or W around target) and attemps to move towards/into one.
			To move the unit finds the nearest square towards the target (N, S, E, W ) and moves to it.
				In the case of ties the unit moves to the first closest square in reading order.
				Once moved, the unit's turn ends.
			If no move can be made and the unit is out of range the unit's turn end
