...
class Game( ):
	def __init__( self, player_count: int, final_marble_value: int ):
		self._player_count = player_count
		self._players = defaultdict( list )
		self._final_marble_value = final_marble_value
	...
	def _calculate_score( self, player: int ):
		print( "The player who put down the final marble is: ", player )

	
	def _turn( self, player: int ):
		self._marble += 1
		insert_location = self._current_marble_idx + 2
		if insert_location > len( self._circle ):
			insert_location = len( self._circle ) - insert_location
		...
		if self._marble == self._final_marble_value:
			self._calculate_score( player )


	def play( self ):
		for p in itertools.cycle( range( 1, self._player_count + 1 ) ):
			self._turn( p )
...
if __name__ == '__main__':
	...
	game = Game( player_count, final_marble_value )
	game.play( )